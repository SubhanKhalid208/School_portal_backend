...
// 6. Report Card (Fixed Aliasing for Date)
export const getQuizResult = async (req, res) => {
    const { assignment_id } = req.params;
    const studentId = req.user.id;
    try {
        const result = await pool.query(`
            SELECT qr.*, q.title, q.passing_marks, u.name as teacher_name, qr.created_at AS submitted_at
...
export const getTeacherQuizResults = async (req, res) => {
    ...
    try {
        const query = `
            SELECT u.name as student_name, u.email as student_email, qr.score, qr.total_marks, qr.status, qr.created_at as submitted_at
            FROM quiz_results qr
            JOIN quiz_assignments qa ON qr.assignment_id = qa.id
